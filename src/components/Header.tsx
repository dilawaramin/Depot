...
// We do NOT export a height variable anymore

const Header: React.FC = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
  // New state to track if the banner is rendered or not
  ...
  const handleBannerVisibleChange = (isVisible: boolean) => {
    // This updates the local state, allowing us to conditionally render the padding
  ...
  const linkClass = (path: string) =>
    ...
    ${
      location.pathname === path 
        ? 'text-green-900 bg-white font-semibold shadow-sm'
        : 'text-white/95 hover:text-green-900 hover:bg-white/70'
    }`;
  ...
  return (
    ...
    <div className="fixed top-0 w-full z-50"> 
      ...
      {/* 2. The Banner is the first element, sitting flush against top-0 */}
      <UpdateBanner onVisibleChange={handleBannerVisibleChange} /> 
                ...
                <Link
                  ...
                  className={`block w-full text-left font-medium text-base px-3 py-2 rounded-md transition-colors transition-all duration-200 ${
                    location.pathname === item.path 
                      ? 'text-green-900 bg-white font-semibold'
                      : 'text-white/95 hover:text-green-900 hover:bg-white/70'
                  }`}
                >
                  {item.label}
